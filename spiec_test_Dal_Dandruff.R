...
OTUTableDCA <- read_tsv("OTU_Bacterial_Dalian_Scalp_Dandruff.txt")
# cast OTU table
OTUTableDCA <- dcast(OTUTableDCA, Sample ~ OTU, value.var = "Count", fill = 0)
OTUTableDCA[1] <- NULL
# select most prevalent OTUs by occurrence frequency
OTUTableDCA <- 1*(OTUTableDCA>0)
freqDCA <- apply(OTUTableDCA, 2, mean)
freqDCA <- freqDCA[freqDCA >=0.25]
freqDCA <- as.matrix(freqDCA)
abundOTUsDCA <- rownames(freqDCA)
prevalenceDCA <- freqDCA[,1]
...
OTUTableDCA <- dcast(OTUTableDCA, Sample ~ OTU, value.var = "Count", fill = 0)
OTUTableDCA[1] <- NULL
N <- apply(OTUTableDCA,1,sum)
OTUTableDCA <- read_tsv("OTU_Bacterial_Dalian_Scalp_Dandruff.txt") %>% filter(OTU %in% abundOTUsDCA)
OTUTableDCA <- dcast(OTUTableDCA, Sample ~ OTU, value.var = "Count", fill = 0)
OTUTableDCA[1] <- NULL
abundDCA <- OTUTableDCA/N
abundDCA <- apply(abundDCA,2,mean)
abundanceDCA <- abundDCA

#Store taxonomic information of prevalent OTUs (bacteria)
OTUTableDCA <- read_tsv("OTU_Bacterial_Dalian_Scalp_Dandruff.txt") %>%
filter(OTU %in% abundOTUsDCA) %>%
select(OTU, Kingdom, Phylum, Class, Order, Family, Genus) %>%
unique()
GenusDCA <- OTUTableDCA$Genus
ClassDCA <- OTUTableDCA$Class
PhylumDCA <- OTUTableDCA$Phylum
KingdomDCA <- OTUTableDCA$Kingdom

#Prepare phyloseq OTU table with prevalent OTUs (bacteria)
OTUTableDCA <- "OTU_Bacterial_Dalian_Scalp_Dandruff.txt" %>%
read_tsv %>%
filter(OTU %in% abundOTUsDCA) %>%
...
TaxaTableDCA <- "OTU_Bacterial_Dalian_Scalp_Dandruff.txt" %>%
read_tsv %>%
filter(OTU %in% abundOTUsDCA) %>%
...
OTUTableFCA <- read_tsv("OTU_Fungal_Dalian_Scalp_Dandruff.txt")
# cast OTU table
...
OTUTableFCA <- read_tsv("OTU_Fungal_Dalian_Scalp_Dandruff.txt") %>% filter(OTU %in% abundOTUsFCA)
OTUTableFCA <- dcast(OTUTableFCA, Sample ~ OTU, value.var = "Count", fill = 0)
OTUTableFCA[1] <- NULL
...
OTUTableFCA <- "OTU_Fungal_Dalian_Scalp_Dandruff.txt" %>%
read_tsv %>%
filter(OTU %in% abundOTUsFCA) %>%
select(OTU, Sample, Count) %>%
mutate(OTU = word(OTU, 1, sep = ";")) %>%
...
TaxaTableFCA <- "OTU_Fungal_Dalian_Scalp_Dandruff.txt" %>%
read_tsv %>%
filter(OTU %in% abundOTUsFCA) %>%
select(OTU, Domain = Kingdom, Phylum, Class, Order, Family, Genus, Species) %>%
mutate(OTU = word(OTU, 1, sep = ";")) %>%
...
data_clr_combined_dca <- cbind(data_clr_dca,data_clr_fca)
...
freqDCA <- c(freqDCA,freqFCA)
abundDCA <- c(abundDCA,abundFCA)
prevalenceDCA <- freqDCA
abundanceDCA <- abundDCA
GenusCombinedDCA <- c(GenusDCA,GenusFCA)
ClassCombinedDCA <- c(ClassDCA,ClassFCA)
PhylumCombinedDCA <- c(PhylumDCA,PhylumFCA)
KingdomCombinedDCA <- c(KingdomDCA,KingdomFCA)
...
sparseiCov_dca <- sparseiCov(data_clr_combined_dca, method="mb", lambda.min.ratio=1e-3, verbose=TRUE, nlambda=20)
...
spiec.easi.combined_dca <- icov.select(sparseiCov_dca, rep.num = 20, ncores = 12, verbose=TRUE)

#Merge bacterial and fungal phyloseq
PhyseqCombined_DC_Dandruff <- merge_phyloseq(PhyseqDCA,PhyseqFCA)

# 6 Convert to igraph and network objects
ig.combined_dca <- spiec.easi.combined_dca %>%
.$refit %>%
adj2igraph(vertex.attr = list(name = taxa_names(PhyseqCombined_DC_Dandruff)))

vertex_attr(ig.combined_dca)
edge_attr(ig.combined_dca)

betweenness_dca <- betweenness(ig.combined_dca)
ig.combined_dca <- set_vertex_attr(ig.combined_dca,"betweenness",value=betweenness_dca)
degree_dca <- degree(ig.combined_dca)
ig.combined_dca <- set_vertex_attr(ig.combined_dca,"degree", value=degree_dca)


# set OTUs vertex attributes
ig.combined_dca <- set_vertex_attr(ig.combined_dca, "prevalence", value=prevalenceDCA)
ig.combined_dca <- set_vertex_attr(ig.combined_dca, "abundance", value=abundanceDCA)
GenusCombinedDCA[is.na(GenusCombinedDCA)] <- "unclassified_Genus"
ig.combined_dca <- set_vertex_attr(ig.combined_dca, "Genus", value=GenusCombinedDCA)
ClassCombinedDCA[is.na(ClassCombinedDCA)] <- "unclassified_Class"
ig.combined_dca <- set_vertex_attr(ig.combined_dca, "Class", value=ClassCombinedDCA)
ig.combined_dca <- set_vertex_attr(ig.combined_dca, "Phylum", value=PhylumCombinedDCA)
ig.combined_dca <- set_vertex_attr(ig.combined_dca, "Kingdom", value=KingdomCombinedDCA)

#Extrac OTU names for later use
OTUNamescombined_DC_Dandruff <- PhyseqCombined_DC_Dandruff %>% otu_table %>% colnames

# 7 Determine correlation weights and direction (positive or negative
# correlation), and save
elist.weighted.combined_dca <- spiec.easi.combined_dca %>%
getOptBeta %>%
symBeta(mode = 'maxabs') %>%
summary %>%
# Assuming OTU numbers in i and j correlate to the order of OTUs in
# se.summer3zw, restore OTU names
left_join(data.frame(OTU1 = OTUNamescombined_DC_Dandruff, i = 1:length(OTUNamescombined_DC_Dandruff))) %>%
left_join(data.frame(OTU2 = OTUNamescombined_DC_Dandruff, j = 1:length(OTUNamescombined_DC_Dandruff)))

elist.weighted.combined_dca <- elist.weighted.combined_dca %>%
mutate(CorSign = ifelse(x > 0, "Positive", "Negative")) %>%
mutate(CorMagnitude = abs(x))

#Save elist document
write.tidy(elist.weighted.combined_dca,"elist.weighted.combined_dca.txt")

ig.combined_dca <- set_edge_attr(ig.combined_dca, "CorrelationSign", value=elist.weighted.combined_dca$CorSign)
ig.combined_dca <- set_edge_attr(ig.combined_dca, "CorrelationMagnitude", value=elist.weighted.combined_dca$CorMagnitude)


# remove weak edges (< 0.15)
Weak = which(E(ig.combined_dca)$CorrelationMagnitude<0.15)
ig.combined_dca = delete_edges(ig.combined_dca, Weak)

# remove vertices without strong edges (independent vertices)
Isolated = which(degree(ig.combined_dca)==0)
ig.combined_dca = delete_vertices(ig.combined_dca, Isolated)

# plot the network
library(RColorBrewer)
Cor <- E(ig.combined_dca)$CorrelationMagnitude

par(mfrow=c(1,2))
coords <- layout.fruchterman.reingold(ig.combined_dca)
E(ig.combined_dca)$color[E(ig.combined_dca)$CorrelationSign == "Negative"] <- 'red'
E(ig.combined_dca)$color[E(ig.combined_dca)$CorrelationSign == "Positive"] <- 'blue'

pal <- brewer.pal(length(unique(ig.combined_dca$Phylum)), "Set3")
...
plot(ig.combined_dca,
     vertex.label=NA,
     vertex.size= sqrt(V(ig.combined_dca)$abundance*500)+2,
     vertex.color= pal2[as.numeric(as.factor(vertex_attr(ig.combined_dca, "Kingdom")))],
     edge.width=Cor*4,
     layout=coords)
legend("topleft", legend=levels(as.factor(V(ig.combined_dca)$Kingdom)), bg= "#e1e5de",col = pal2, bty="o",pch=20, pt.cex = 1.3, cex = 0.45, text.col=pal2 , horiz = FALSE, inset = c(0.3, 0.83))
title("Dalian Scalp - Dandruff", line=-1)

# Perform fast-greedy community detection on network graph
kc = fastgreedy.community(ig.combined_dca)

# Perform edge-betweenness community detection on network graph
gc = edge.betweenness.community(ig.combined_dca)
...
my_color =pal[as.numeric(as.factor(V(ig.combined)$ClassCombinedDCA))]
pal2 <- c("red", "grey")
my_color2 =pal2[as.numeric(as.factor(E(ig.combined)$CorrelationSignDCA))]


par(mfrow = c(1, 2))
plot(kc, ig.combined_dca,
vertex.label = NA,
vertex.size = 5,
mark.groups=kc,
...
pdf("degree_distribution_scalp.pdf",width=4,height=4)
par(mar=c(4,4,4,4))
plot(seq_along(dd.BCH)-1, dd.BCH, type='b', xlim=c(0,30), ylim=c(0,0.3),
...
legend("topright", c("Baoding Healthy","Baoding Dandruff","Dalian Healthy", "Dalian Dandruff"), col=c("blue","green","red","black"),pch=1, lty=1, cex=0.6)
title(main="Scalp")
dev.off()
