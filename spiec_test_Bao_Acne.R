...
OTUTableBCA <- read_tsv("OTU_Table_Bacterial_Baoding_Acne.txt")
# cast OTU table
OTUTableBCA <- dcast(OTUTableBCA, Sample ~ OTU, value.var = "Count", fill = 0)
OTUTableBCA[1] <- NULL
# select most prevalent OTUs by occurrence frequency
OTUTableBCA <- 1*(OTUTableBCA>0)
freqBCA <- apply(OTUTableBCA, 2, mean)
freqBCA <- freqBCA[freqBCA >=0.25]
freqBCA <- as.matrix(freqBCA)
abundOTUsBCA <- rownames(freqBCA)
prevalenceBCA <- freqBCA[,1]
...
OTUTableBCA <- dcast(OTUTableBCA, Sample ~ OTU, value.var = "Count", fill = 0)
OTUTableBCA[1] <- NULL
N <- apply(OTUTableBCA,1,sum)
OTUTableBCA <- read_tsv("OTU_Table_Bacterial_Baoding_Acne.txt") %>% filter(OTU %in% abundOTUsBCA)
OTUTableBCA <- dcast(OTUTableBCA, Sample ~ OTU, value.var = "Count", fill = 0)
OTUTableBCA[1] <- NULL
abundBCA <- OTUTableBCA/N
abundBCA <- apply(abundBCA,2,mean)
abundanceBCA <- abundBCA

#Store taxonomic information of prevalent OTUs (bacteria)
OTUTableBCA <- read_tsv("OTU_Table_Bacterial_Baoding_Acne.txt") %>%
filter(OTU %in% abundOTUsBCA) %>%
select(OTU, Kingdom, Phylum, Class, Order, Family, Genus) %>%
unique()
GenusBCA <- OTUTableBCA$Genus
ClassBCA <- OTUTableBCA$Class
PhylumBCA <- OTUTableBCA$Phylum
KingdomBCA <- OTUTableBCA$Kingdom


#Prepare phyloseq OTU table with prevalent OTUs (bacteria)
OTUTableBCA <- "OTU_Table_Bacterial_Baoding_Acne.txt" %>%
read_tsv %>%
filter(OTU %in% abundOTUsBCA) %>%
...
TaxaTableBCA <- "OTU_Table_Bacterial_Baoding_Acne.txt" %>%
read_tsv %>%
filter(OTU %in% abundOTUsBCA) %>%
...
OTUTableFCA <- read_tsv("OTU_Table_Fungal_Baoding_Acne.txt")
# cast OTU table
OTUTableFCA <- dcast(OTUTableFCA, Sample ~ OTU, value.var = "Count", fill = 0)
OTUTableFCA[1] <- NULL
# select most prevalent OTUs by occurrence frequency
OTUTableFCA <- 1*(OTUTableFCA>0)
freqFCA <- apply(OTUTableFCA, 2, mean)
freqFCA <- freqFCA[freqFCA >=0.25]
freqFCA <- as.matrix(freqFCA)
abundOTUsFCA <- rownames(freqFCA)
prevalenceFCA <- freqFCA[,1]
...
OTUTableFCA <- dcast(OTUTableFCA, Sample ~ OTU, value.var = "Count", fill = 0)
OTUTableFCA[1] <- NULL
N <- apply(OTUTableFCA,1,sum)
OTUTableFCA <- read_tsv("OTU_Table_Fungal_Baoding_Acne.txt") %>% filter(OTU %in% abundOTUsFCA)
OTUTableFCA <- dcast(OTUTableFCA, Sample ~ OTU, value.var = "Count", fill = 0)
OTUTableFCA[1] <- NULL
abundFCA <- OTUTableFCA/N
abundFCA <- apply(abundFCA,2,mean)
abundanceFCA <- abundFCA


#Store taxonomic information (fungi)
OTUTableFCA <- read_tsv("OTU_Table_Fungal_Baoding_Acne.txt") %>%
filter(OTU %in% abundOTUsFCA) %>%
select(OTU, Kingdom, Phylum, Class, Order, Family, Genus) %>%
unique()
GenusFCA <- OTUTableFCA$Genus
ClassFCA <- OTUTableFCA$Class
PhylumFCA <- OTUTableFCA$Phylum
KingdomFCA <- OTUTableFCA$Kingdom

#Prepare phyloseq OTU table (fungi)
OTUTableFCA <- "OTU_Table_Fungal_Baoding_Acne.txt" %>%
read_tsv %>%
filter(OTU %in% abundOTUsFCA) %>%
select(OTU, Sample, Count) %>%
mutate(OTU = word(OTU, 1, sep = ";")) %>%
...
TaxaTableFCA <- "OTU_Table_Fungal_Baoding_Acne.txt" %>%
read_tsv %>%
filter(OTU %in% abundOTUsFCA) %>%
select(OTU, Domain = Kingdom, Phylum, Class, Order, Family, Genus, Species) %>%
mutate(OTU = word(OTU, 1, sep = ";")) %>%
...
freqBCA <- c(freqBCA,freqFCA)
abundBCA <- c(abundBCA,abundFCA)
prevalenceBCA <- freqBCA
abundanceBCA <- abundBCA
GenusCombinedBCA <- c(GenusBCA,GenusFCA)
ClassCombinedBCA <- c(ClassBCA,ClassFCA)
PhylumCombinedBCA <- c(PhylumBCA,PhylumFCA)
KingdomCombinedBCA <- c(KingdomBCA,KingdomFCA)
...
sparseiCov_bca <- sparseiCov(data_clr_combined_bca, method="mb", lambda.min.ratio=1e-3, verbose=TRUE, nlambda=20)
...
spiec.easi.combined_bca <- icov.select(sparseiCov_bca, rep.num = 20, ncores = 10, verbose=TRUE)

#Merge bacterial and fungal phyloseq
PhyseqCombined_DC_Acne <- merge_phyloseq(PhyseqBCA,PhyseqFCA)

# 6 Convert to igraph and network objects
ig.combined_bca <- spiec.easi.combined_bca %>%
.$refit %>%
adj2igraph(vertex.attr = list(name = taxa_names(PhyseqCombined_DC_Acne)))

vertex_attr(ig.combined_bca)
edge_attr(ig.combined_bca)

betweenness_bca <- betweenness(ig.combined_bca)
ig.combined_bca <- set_vertex_attr(ig.combined_bca,"betweenness",value=betweenness_bca)
degree_bca <- degree(ig.combined_bca)
ig.combined_bca <- set_vertex_attr(ig.combined_bca,"degree", value=degree_bca)


# set OTUs vertex attributes
ig.combined_bca <- set_vertex_attr(ig.combined_bca, "prevalence", value=prevalenceBCA)
ig.combined_bca <- set_vertex_attr(ig.combined_bca, "abundance", value=abundanceBCA)
GenusCombinedBCA[is.na(GenusCombinedBCA)] <- "unclassified_Genus"
ig.combined_bca <- set_vertex_attr(ig.combined_bca, "Genus", value=GenusCombinedBCA)
ClassCombinedBCA[is.na(ClassCombinedBCA)] <- "unclassified_Class"
ig.combined_bca <- set_vertex_attr(ig.combined_bca, "Class", value=ClassCombinedBCA)
ig.combined_bca <- set_vertex_attr(ig.combined_bca, "Phylum", value=PhylumCombinedBCA)
ig.combined_bca <- set_vertex_attr(ig.combined_bca, "Kingdom", value=KingdomCombinedBCA)

#Extrac OTU names for later use
OTUNamescombined_DC_Acne <- PhyseqCombined_DC_Acne %>% otu_table %>% colnames

# 7 Determine correlation weights and direction (positive or negative
# correlation), and save
elist.weighted.combined_bca <- spiec.easi.combined_bca %>%
getOptBeta %>%
symBeta(mode = 'maxabs') %>%
summary %>%
# Assuming OTU numbers in i and j correlate to the order of OTUs in
# se.summer3zw, restore OTU names
left_join(data.frame(OTU1 = OTUNamescombined_DC_Acne, i = 1:length(OTUNamescombined_DC_Acne))) %>%
left_join(data.frame(OTU2 = OTUNamescombined_DC_Acne, j = 1:length(OTUNamescombined_DC_Acne)))

elist.weighted.combined_bca <- elist.weighted.combined_bca %>%
mutate(CorSign = ifelse(x > 0, "Positive", "Negative")) %>%
mutate(CorMagnitude = abs(x))

#Save elist document
#write.tidy(elist.weighted.combined_bca,"elist.weighted.combined_bca.txt")

ig.combined_bca <- set_edge_attr(ig.combined_bca, "CorrelationSign", value=elist.weighted.combined_bca$CorSign)
ig.combined_bca <- set_edge_attr(ig.combined_bca, "CorrelationMagnitude", value=elist.weighted.combined_bca$CorMagnitude)


# remove weak edges (< 0.15)
Weak = which(E(ig.combined_bca)$CorrelationMagnitude<0.15)
ig.combined_bca = delete_edges(ig.combined_bca, Weak) 

# remove vertices without strong edges (independent vertices)
Isolated = which(degree(ig.combined_bca)==0)
ig.combined_bca = delete_vertices(ig.combined_bca, Isolated)

# plot the network
library(RColorBrewer)
Cor <- E(ig.combined_bca)$CorrelationMagnitude

par(mfrow=c(1,2))
coords <- layout.fruchterman.reingold(ig.combined_bca)
E(ig.combined_bca)$color[E(ig.combined_bca)$CorrelationSign == "Negative"] <- 'red'
E(ig.combined_bca)$color[E(ig.combined_bca)$CorrelationSign == "Positive"] <- 'blue'
...
plot(ig.combined_bca,
vertex.label=NA,
vertex.size= sqrt(V(ig.combined_bca)$abundance*500)+2,
vertex.color= pal2[as.numeric(as.factor(vertex_attr(ig.combined_bca, "Kingdom")))],
edge.width=Cor*4,
layout=coords)
...
my_color =pal[as.numeric(as.factor(V(ig.combined)$ClassCombinedBCA))]
pal2 <- c("red", "grey")
my_color2 =pal2[as.numeric(as.factor(E(ig.combined)$CorrelationSignBCA))]


par(mfrow = c(1, 2))
plot(kc, ig.combined_bca,
vertex.label = NA,
vertex.size = 5,
mark.groups=kc,
...
png("degree_distribution_cheek.png",width=4,height=4,units="in",res=2000)
par(mar=c(4,4,4,4))
plot(seq_along(dd.BCH)-1, dd.BCH, type='b', xlim=c(0,10), ylim=c(0,1), ylab="Frequency", xlab="Degree", col='blue')
...
nc.attack <- function(ig) {
  ...
  sapply(1:round(vcount(ig)*.7), function(i) {
    ind <- hubord[1:i]
...
nc.BCA <- nc.attack(ig.combined_bca)
nc.DCH <- nc.attack(ig.combined_dch)
nc.DCA <- nc.attack(ig.combined_dca)

write.tidy(nc.BCH,"Attack_BCH.txt")
write.tidy(nc.BCA,"Attack_BCA.txt")
write.tidy(nc.DCH,"Attack_DCH.txt")
write.tidy(nc.DCA,"Attack_DCA.txt")


##Plot Natural connectivity

png("connectivity_cheek.png",width=4,height=4,units="in",res=1200)
plot(seq(0,0.8,len=length(nc.BCH)),nc.BCH, type='l', col='blue', ylim=c(0,max(nc.DCA)), xlab="Proportion of Removed Nodes", ylab="Natural Connectivity",font.lab=2)
points(seq(0,0.8,len=length(nc.BCA)),nc.BCA,type='l', col='green')
points(seq(0,0.8,len=length(nc.DCH)),nc.DCH,type='l', col='red')
points(seq(0,0.8,len=length(nc.DCA)),nc.DCA,type='l', col='black')
legend("topright", c("Baoding Healthy","Baoding Acne","Dalian Healthy", "Dalian Acne"), col=c("blue","green","red","black"),pch=1, lty=1, cex=0.6)
title(main="Cheek")
dev.off()
