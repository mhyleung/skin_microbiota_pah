...
OTUTableDCA <- read_tsv("OTU_Bacterial_Baoding_Scalp_Dandruff.txt")
# cast OTU table
...
OTUTableDCA <- read_tsv("OTU_Bacterial_Baoding_Scalp_Dandruff.txt") %>% filter(OTU %in% abundOTUsDCA)
OTUTableDCA <- dcast(OTUTableDCA, Sample ~ OTU, value.var = "Count", fill = 0)
OTUTableDCA[1] <- NULL
...
OTUTableDCA <- "OTU_Bacterial_Baoding_Scalp_Dandruff.txt" %>%
read_tsv %>%
filter(OTU %in% abundOTUsDCA) %>%
...
TaxaTableDCA <- "OTU_Bacterial_Baoding_Scalp_Dandruff.txt" %>%
read_tsv %>%
filter(OTU %in% abundOTUsDCA) %>%
...
OTUTableFCA <- read_tsv("OTU_Fungal_Baoding_Scalp_Dandruff.txt")
# cast OTU table
...
OTUTableFCA <- read_tsv("OTU_Fungal_Baoding_Scalp_Dandruff.txt") %>% filter(OTU %in% abundOTUsFCA)
OTUTableFCA <- dcast(OTUTableFCA, Sample ~ OTU, value.var = "Count", fill = 0)
OTUTableFCA[1] <- NULL
...
OTUTableFCA <- "OTU_Fungal_Baoding_Scalp_Dandruff.txt" %>%
read_tsv %>%
filter(OTU %in% abundOTUsFCA) %>%
select(OTU, Sample, Count) %>%
mutate(OTU = word(OTU, 1, sep = ";")) %>%
...
TaxaTableFCA <- "OTU_Fungal_Baoding_Scalp_Dandruff.txt" %>%
read_tsv %>%
filter(OTU %in% abundOTUsFCA) %>%
select(OTU, Domain = Kingdom, Phylum, Class, Order, Family, Genus, Species) %>%
mutate(OTU = word(OTU, 1, sep = ";")) %>%
...
spiec.easi.combined_dca <- icov.select(sparseiCov_dca, rep.num = 20, ncores = 10, verbose=TRUE)
...
title("Baoding Scalp - Dandruff", line=-1)
...
ig.combined_bca <- ig.combined_dca
...
png("degree_distribution_scalp.png",width=4,height=4,units="in",res=1200)
par(mar=c(4,4,4,4))
...
legend("topright", c("Baoding Healthy","Baoding Dandruff","Baoding Healthy", "Baoding Dandruff"), col=c("blue","green","red","black"),pch=1, lty=1, cex=0.6)
title(main="Cheek")
dev.off()
