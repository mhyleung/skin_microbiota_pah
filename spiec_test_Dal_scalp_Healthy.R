...
OTUTableDCH <- read_tsv("OTU_Bacterial_Dalian_Scalp_Healthy.txt")
# cast OTU table
...
OTUTableDCH <- read_tsv("OTU_Bacterial_Dalian_Scalp_Healthy.txt") %>% filter(OTU %in% abundOTUsDCH)
OTUTableDCH <- dcast(OTUTableDCH, Sample ~ OTU, value.var = "Count", fill = 0)
OTUTableDCH[1] <- NULL
...
OTUTableDCH <- "OTU_Bacterial_Dalian_Scalp_Healthy.txt" %>%
read_tsv %>%
filter(OTU %in% abundOTUsDCH) %>%
...
TaxaTableDCH <- "OTU_Bacterial_Dalian_Scalp_Healthy.txt" %>%
read_tsv %>%
filter(OTU %in% abundOTUsDCH) %>%
...
OTUTableFCH <- read_tsv("OTU_Fungal_Dalian_Scalp_Healthy.txt")
# cast OTU table
...
OTUTableFCH <- read_tsv("OTU_Fungal_Dalian_Scalp_Healthy.txt") %>% filter(OTU %in% abundOTUsFCH)
OTUTableFCH <- dcast(OTUTableFCH, Sample ~ OTU, value.var = "Count", fill = 0)
OTUTableFCH[1] <- NULL
...
OTUTableFCH <- "OTU_Fungal_Dalian_Scalp_Healthy.txt" %>%
read_tsv %>%
filter(OTU %in% abundOTUsFCH) %>%
select(OTU, Sample, Count) %>%
mutate(OTU = word(OTU, 1, sep = ";")) %>%
...
TaxaTableFCH <- "OTU_Fungal_Dalian_Scalp_Healthy.txt" %>%
read_tsv %>%
filter(OTU %in% abundOTUsFCH) %>%
select(OTU, Domain = Kingdom, Phylum, Class, Order, Family, Genus, Species) %>%
mutate(OTU = word(OTU, 1, sep = ";")) %>%
...
spiec.easi.combined_dch <- icov.select(sparseiCov_dch, rep.num = 20, ncores = 12, verbose=TRUE)
...
pal1 = colorRampPalette(c('#deebf7','#08519c'))
#this gives you the colors you want for every point
graphCol1 = pal1(fine)[as.numeric(cut(V(ig.combined_bch)$degree,breaks = fine))]
# now you just need to plot it with those colors
#plot(adj, vertex.color=graphCol)

pal2 = colorRampPalette(c('#e5f5e0','#006d2c'))
#this gives you the colors you want for every point
graphCol2 = pal2(fine)[as.numeric(cut(V(ig.combined_bch)$prevalence,breaks = fine))]
...
vertex.label=NA,
vertex.size=(V(ig.combined_bch)$betweenness/250)+2,
vertex.color=graphCol1,
vertex.frame.color= "black",
edge.width=Cor*4,
#layout_nicely(ig.summer),
main = "color intensity=degree, size=betweeness centrality",
layout=coords)

plot(ig.combined_bch,
vertex.label=NA,
vertex.size= sqrt(V(ig.combined_bch)$abundance*500)+2,
vertex.color=graphCol2,
vertex.frame.color= "black",
edge.width=Cor*4,
#layout_nicely(ig.summer),
main = "color intensity=prevalence, size=abundance",
layout=coords)

pal <- brewer.pal(length(unique(ig.combined_bca$Phylum)), "Set3")

pal2 <- c("#1f78b4","#b2df8a", "#33a02c", "#fb9a99", "#e31a1c", "#fdbf6f", "#ff7f00", "#c41185",  "#320956", "#b15928","black","purple","blue", "red","green","brown","orange")

fine = 2000 # this will adjust the resolving power.
plot(ig.combined_bch,
     vertex.label=NA,
     vertex.size= sqrt(V(ig.combined_bch)$abundance*500)+2,
     vertex.color= pal2[as.numeric(as.factor(vertex_attr(ig.combined_bch, "Phylum")))],
     edge.width=Cor*4,
     layout=coords)
legend("topleft", legend=levels(as.factor(V(ig.combined_bch)$Phylum)), bg= "#e1e5de",col = pal2, bty="o",pch=20, pt.cex = 1.3, cex = 0.45, text.col=pal2 , horiz = FALSE, inset = c(0.3, 0.83))
...
my_color =pal[as.numeric(as.factor(V(ig.combined)$ClassCombinedBCH))]
pal2 <- c("red", "grey")
my_color2 =pal2[as.numeric(as.factor(E(ig.combined)$CorrelationSignBCH))]
