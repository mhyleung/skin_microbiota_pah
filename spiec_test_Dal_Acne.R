...
OTUTableDCA <- read_tsv("OTU_Table_Bacterial_Dalian_Acne.txt")
# cast OTU table
...
OTUTableDCA <- read_tsv("OTU_Table_Bacterial_Dalian_Acne.txt") %>% filter(OTU %in% abundOTUsDCA)
OTUTableDCA <- dcast(OTUTableDCA, Sample ~ OTU, value.var = "Count", fill = 0)
OTUTableDCA[1] <- NULL
...
OTUTableDCA <- "OTU_Table_Bacterial_Dalian_Acne.txt" %>%
read_tsv %>%
filter(OTU %in% abundOTUsDCA) %>%
...
TaxaTableDCA <- "OTU_Table_Bacterial_Dalian_Acne.txt" %>%
read_tsv %>%
filter(OTU %in% abundOTUsDCA) %>%
...
OTUTableFCA <- read_tsv("OTU_Table_Fungal_Dalian_Acne.txt")
# cast OTU table
...
OTUTableFCA <- read_tsv("OTU_Table_Fungal_Dalian_Acne.txt") %>% filter(OTU %in% abundOTUsFCA)
OTUTableFCA <- dcast(OTUTableFCA, Sample ~ OTU, value.var = "Count", fill = 0)
OTUTableFCA[1] <- NULL
...
OTUTableFCA <- "OTU_Table_Fungal_Dalian_Acne.txt" %>%
read_tsv %>%
filter(OTU %in% abundOTUsFCA) %>%
select(OTU, Sample, Count) %>%
mutate(OTU = word(OTU, 1, sep = ";")) %>%
...
TaxaTableFCA <- "OTU_Table_Fungal_Dalian_Acne.txt" %>%
read_tsv %>%
filter(OTU %in% abundOTUsFCA) %>%
select(OTU, Domain = Kingdom, Phylum, Class, Order, Family, Genus, Species) %>%
mutate(OTU = word(OTU, 1, sep = ";")) %>%
...
spiec.easi.combined_dca <- icov.select(sparseiCov_dca, rep.num = 20, ncores = 16, verbose=TRUE)

#Merge bacterial and fungal phyloseq
PhyseqCombined_DC_Acne <- merge_phyloseq(PhyseqDCA,PhyseqFCA)
...
pal <- brewer.pal(length(unique(ig.combined_dca$Kingdom)), "Set3")
...
edge.width=Cor*4,
#layout_nicely(ig.summer),
layout=coords)
legend("topleft", legend=levels(as.factor(V(ig.combined_dca)$Kingdom)), bg= "#e1e5de",col = pal2, bty="o",pch=20, pt.cex = 1.3, cex = 0.6, text.col=pal2 , horiz = FALSE, inset = c(0.3, 0.87))
title("Dalian Cheek - Acne", line=-1)
...
pdf("degree_distribution_cheek.pdf",width=5,height=5)
par(mar=c(4,4,4,4))
