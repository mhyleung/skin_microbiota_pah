...
OTUTableBCH <- read_tsv("OTU_Table_Bacterial_Baoding_Healthy.txt")
# cast OTU table
...
OTUTableBCH <- read_tsv("OTU_Table_Bacterial_Baoding_Healthy.txt") %>% filter(OTU %in% abundOTUsBCH)
OTUTableBCH <- dcast(OTUTableBCH, Sample ~ OTU, value.var = "Count", fill = 0)
OTUTableBCH[1] <- NULL
...
OTUTableBCH <- "OTU_Table_Bacterial_Baoding_Healthy.txt" %>%
read_tsv %>%
filter(OTU %in% abundOTUsBCH) %>%
...
TaxaTableBCH <- "OTU_Table_Bacterial_Baoding_Healthy.txt" %>%
read_tsv %>%
filter(OTU %in% abundOTUsBCH) %>%
...
OTUTableFCH <- read_tsv("OTU_Table_Fungal_Baoding_Healthy.txt")
# cast OTU table
...
OTUTableFCH <- read_tsv("OTU_Table_Fungal_Baoding_Healthy.txt") %>% filter(OTU %in% abundOTUsFCH)
OTUTableFCH <- dcast(OTUTableFCH, Sample ~ OTU, value.var = "Count", fill = 0)
OTUTableFCH[1] <- NULL
...
OTUTableFCH <- "OTU_Table_Fungal_Baoding_Healthy.txt" %>%
read_tsv %>%
filter(OTU %in% abundOTUsFCH) %>%
select(OTU, Sample, Count) %>%
mutate(OTU = word(OTU, 1, sep = ";")) %>%
...
TaxaTableFCH <- "OTU_Table_Fungal_Baoding_Healthy.txt" %>%
read_tsv %>%
filter(OTU %in% abundOTUsFCH) %>%
select(OTU, Domain = Kingdom, Phylum, Class, Order, Family, Genus, Species) %>%
mutate(OTU = word(OTU, 1, sep = ";")) %>%
...
spiec.easi.combined_bch <- icov.select(sparseiCov_bch, rep.num = 20, ncores = 13, verbose=TRUE)

#Merge bacterial and fungal phyloseq
PhyseqCombined_DC_Healthy <- merge_phyloseq(PhyseqBCH,PhyseqFCH)
...
pal <- brewer.pal(length(unique(ig.combined_bch$Phylum)), "Set3")
...
edge.width=Cor*4,
layout=coords)
legend("topleft", legend=levels(as.factor(V(ig.combined_bch)$Kingdom)), bg= "#e1e5de",col = pal2, bty="o",pch=20, pt.cex = 1.3, cex = 0.45, text.col=pal2 , horiz = FALSE, inset = c(0.3, 0.83))
title("Baoding Cheek - Healthy", line=-1)
...
pdf("connectivity_cheek.png",width=4,height=4)
plot(seq(0,0.8,len=length(nc.BCH)),nc.BCH, type='l', col='blue', ylim=c(0,max(nc.BCA)), xlab="Proportion of Removed Nodes", ylab="Natural Connectivity",font.lab=2)
